...
from airflow.providers.ssh.hooks.ssh import SSHHook  # <--- THÊM DÒNG NÀY
...
# Thay vì truyền user/pass vào Operator, ta tạo một Hook
ssh_hook = SSHHook(
    remote_host=SPARK_MASTER_HOST,
    username=SPARK_USER,
    password=SPARK_PASSWORD,
    port=22,
    keepalive_interval=10  # Giữ kết nối ổn định
)
...
cmd_producer_1 = """
echo "Waiting 21s for Consumer to warm up..."
sleep 21s
cd /app
export PYTHONPATH=$PYTHONPATH:/app
python3 src/producer/producer.py \
    --input data/traffic_1.mp4 \
    --cam_id CAM_SSH_01 \
...
cmd_producer_2 = """
echo "Waiting 20s for Consumer to warm up..."
sleep 20s
cd /app
export PYTHONPATH=$PYTHONPATH:/app
python3 src/producer/producer.py \
    --input data/traffic_2.mp4 \
    --cam_id CAM_SSH_02 \
    --topic vehicle_stream \
    --server kafka:29092
"""
...
# Dùng spark-submit để nạp dependencies
spark-submit \
    ...
    /app/src/processing/consumer.py
"""
...
run_producer_1_ssh = SSHOperator(
    task_id='run_producer_1_ssh',
    ssh_hook=ssh_hook,
    command=cmd_producer_1,
    cmd_timeout=None,
    dag=dag,
)

run_producer_2_ssh = SSHOperator(
    task_id='run_producer_2_ssh',
    ssh_hook=ssh_hook,
    command=cmd_producer_2,
    cmd_timeout=None,
    dag=dag,
)
...
task_check_postgres >> run_producer_1_ssh
task_check_postgres >> run_producer_2_ssh